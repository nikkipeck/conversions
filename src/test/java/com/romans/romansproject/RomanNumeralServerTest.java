...
public class RomanNumeralServerTest {
    ...
    public void testOutofRangeQuery() {
        try {
            ...
            assertEquals("query value must be of type int values 1-3,999. Query string format example: query={11}",
                    response.body());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void testSuccessfulRange() {
        try {
            ...
            assertEquals(200, response.statusCode());
            assertEquals(
                    "{\"conversions\":[{\"input\":\"4\",\"output\":\"IV\"},{\"input\":\"5\",\"output\":\"V\"},{\"input\":\"6\",\"output\":\"VI\"},{\"input\":\"7\",\"output\":\"VII\"}]}",
                    response.body());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void testBadRange() {
        try {
            ...
            HttpResponse response = getServerResponse(url);
            assertEquals(400, response.statusCode());
            assertEquals("min and max values must be of type int. Query string format example: min={1}&max={3}",
                    response.body());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    /*
     * getServerResponse(Url url) helper method to return an HttpResponse from the
    ...
    private HttpResponse getServerResponse(URL url) {
        ...
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder().version(Version.HTTP_1_1)
                    .uri(URI.create(uri.toASCIIString())).build();
