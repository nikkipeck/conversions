...
/**
 * The RomanNumeralServer class sets up a basic HttpServer to listen on port 8080
 * it serves requests the the romannumeral context
...
public class RomanNumeralServer {
	...
	/*startServer()
	 * starts a com.sun.net.httpserver.HttpServer listening on 8080 a context and a threaded executor
	 * long term this should be switched to a more robust server, leaning on an outside implementation like Jetty
	...
	class RomanNumeralHandler implements HttpHandler{
		 ...
		 * @param query is a query string matching min={4}&max={7} format
		 * validates min and max are ints in the allowable range 1-3,999
		...
		private int[] splitQuery(String query) throws UnsupportedEncodingException {
		    ...
		    for (int i=0; i<pairs.length; i++) {
		        ...
		        try {
		        	...
		        	errors.append("min and max values must be of type int. Query string format example: min={1}&max={3}");
		        	return new int[0];
		        }
		    }
		    
		    if(minMax.length != 2)
		    	errors.append("min and max values are required, only min and max are allowed. Query string format example: min={1}&max={3}");
		 ...
		 * @return String representing json payload
		 * example int[4,6] would return
		...
		private String getSinglePayload(String query) {
			...
			try {
				...
				errors.append("query value must be of type int values 1-3,999. Query string format example: query={11}");
				return errors.toString();
			}
		...
		/*
		 * mapIntToRomanJson will take an integer, convert it to a roman numeral string, and return it as a jsonobject
		 * @param int numer to convert
		 * @return json payload
		 * example 3 would return
		 * "{"input":"3","output":"III"}"
		 */
		 ...
		 * @return jsonobject with conversions for range
		 * example int[3,4] would return
		...
		private JsonObject mapListToJson(int[] minMax) {
			...
			List<JsonObject> inners = stream.mapToObj(i -> mapIntToRomanJson(i))
